// Package redactv2 masks known secret values in a byte stream or a string.
//
// Like [github.com/icholy/gritz/internal/redact] it performs no detection:
// callers supply the exact values to mask, and every occurrence of one is
// replaced by a marker naming the secret it stood for. Unlike that package it
// masks through a [Writer] backed by a byte trie rather than a chain of
// transformers, which buys two things:
//
//   - Nothing is held back unless it is a live prefix of some secret, so the
//     masked stream does not trail the raw one between flushes.
//   - One trie walk covers every secret at once, so the cost and the buffering
//     do not grow with the number of secrets or the length of their values.
//
// Matching is leftmost-longest, where [github.com/icholy/gritz/internal/redact]
// applies one whole-stream rule per secret, longest value first. The two agree
// except when the suffix of one secret's value is the prefix of another's: this
// package masks the leftmost value whole and leaves the second value's tail
// beside the marker, where the longest-first chain masks the longer value
// first. Neither order masks both of a pair that overlaps that way.
package redactv2
